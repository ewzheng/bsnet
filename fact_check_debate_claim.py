# This script takes any claim string, uses a debate-focused detect_claim_type() router, calls the Google Fact Check Tools API 
# claims:search endpoint, and prints the textual rating and source from the best matching fact-check result. 
# Google’s API searches a database of fact-checked claims and returns fields like textualRating, publisher.name, url, and reviewDate.
